...
import optparse
import os
from thirdparty.console_width.consle_width import getTerminalSize
...
class DNSBrute:
    ...
    def _load_dns_servers(self):
        dns_servers = []
        with open(os.path.join(paths.DOMAIN_DICT, 'dns_servers.txt')) as f:
            for line in f:
                server = line.strip()
    ...
    def _load_sub_names(self):
        ...
        file = os.path.join(paths.DOMAIN_DICT, self.names_file) if not os.path.exists(
    ...
    def _load_next_sub(self):
        next_subs = []
        with open(os.path.join(paths.DOMAIN_DICT, 'next_sub.txt')) as f:
            for line in f:
                sub = line.strip()
...
def bruteInterface():
    global d
    d = DNSBrute(target=conf.TARGET_DOMAIN,
                 names_file=os.path.join(paths.DOMAIN_DICT, 'subnames.txt'),
                 ignore_intranet=True,
                 threads_num=50,
                 output=paths.DOMAIN_OUTPUT_PATH)
    d.run()
